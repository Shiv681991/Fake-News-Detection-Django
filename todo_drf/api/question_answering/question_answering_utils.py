...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
            "NbestPrediction", ["text", "text_extended","start_logit", "end_logit"])
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:  # this is a non-null prediction
                ...
                final_text,final_text_extended = get_final_text(
            ...
            nbest.append(
                _NbestPrediction(
                    text=final_text,
                    text_extended=final_text_extended,
        ...
        if version_2_with_negative:
            ...
            if len(nbest) == 1:
                nbest.insert(0,
                             _NbestPrediction(text="empty",text_extended="empty", start_logit=0.0, end_logit=0.0))
...
def write_predictions_extended(all_examples, all_features, all_results, n_best_size,
    ...
    _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
        "NbestPrediction", ["text", "text_extended","start_log_prob", "end_log_prob"])
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for pred in prelim_predictions:
            ...
            final_text,final_text_extended = get_final_text(tok_text, orig_text, False,
                                        verbose_logging)
        ...
        if not nbest:
            nbest.append(
                _NbestPrediction(text="",text_extended="", start_log_prob=-1e6,
                                 end_log_prob=-1e6))
...
def get_best_predictions_extended(all_examples, all_features, all_results, n_best_size,
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for pred in prelim_predictions:
            ...
            final_text,final_text_extended = get_final_text(tok_text, orig_text, tokenizer.do_lower_case,
                                        verbose_logging)
        ...
        for (i, entry) in enumerate(nbest):
            output = collections.OrderedDict()
            output["text"] = entry.text
            output["text_extended"]= entry.text_extended
            output["probability"] = probs[i]
            output["start_log_prob"] = entry.start_log_prob
            output["end_log_prob"] = entry.end_log_prob
            nbest_json.append(output)
        ...
        all_nbest_json[example.qas_id] = nbest_json
        # chaitanya
        all_best = [{'id': id, 'answer': answers[0]['text'],'answer_extended':answers[0]['text_extended']} for id, answers in all_nbest_json.items()]
    return all_best
...
def get_final_text(pred_text, orig_text, do_lower_case, verbose_logging=False):
    ...
    output_text_extended = orig_text[orig_start_position-5:(orig_end_position + 1+5)]
    # return output_text,output_text_extended
    return output_text_extended,output_text
...
def build_single_example(context,question):
    """
    Builds a list of dicts in input data format from a list of contexts and qas.
    """
    examples = []
