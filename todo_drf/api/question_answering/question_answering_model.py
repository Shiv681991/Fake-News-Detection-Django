...
from simpletransformers.question_answering.question_answering_utils import (
    ...
    to_list,
    build_examples,build_single_example,
...
class QuestionAnsweringModel:
    ...
    def load_and_cache_examples(self, examples, evaluate=False, no_cache=False, output_examples=False):
        ...
        cached_features_file = os.path.join(args["cache_dir"], "cached_{}_{}_{}_{}".format(mode, args["model_type"],
                                                                                           args["max_seq_length"],
                                                                                           len(examples)))
        ...
        if os.path.exists(cached_features_file) and not args["reprocess_input_data"] and not no_cache:
            ...
            features = convert_examples_to_features(examples=examples,
                                                    ...
                                                    sequence_a_is_doc=True if args['model_type'] in [
                                                        'xlnet'] else False,
                                                    silent=args['silent']
                                                    )
    ...
    def evaluate(self, eval_data, output_dir):
        ...
        eval_dataset, examples, features = self.load_and_cache_examples(eval_examples, evaluate=True,
        ...
        if args['model_type'] in ['xlnet', 'xlm','albert']:
            # XLNet uses a more complex post-processing procedure
            all_predictions, all_nbest_json, scores_diff_json = write_predictions_extended(examples, features,
                                                                                           all_results,
                                                                                           args['n_best_size'],
            ...
            all_predictions, all_nbest_json, scores_diff_json = write_predictions(examples, features, all_results,
                                                                                  args['n_best_size'],
    ...
    def predict(self, to_predict, n_best_size=None):
        ...
        eval_dataset, examples, features = self.load_and_cache_examples(eval_examples, evaluate=True,
                                                                        output_examples=True, no_cache=True)
        ...
        if args['model_type'] in ['xlnet', 'xlm','albert']:
            ...
            answers = get_best_predictions(examples, features, all_results, n_best_size, args['max_answer_length'],
                                           False, False, True, False)

        return answers

    def predict_single_answer(self, context,question, n_best_size=None):
        """
        Performs predictions on a list of python dicts containing contexts and qas.
        ...
        eval_examples = build_single_example(context,question)
        eval_dataset, examples, features = self.load_and_cache_examples(eval_examples, evaluate=True,
                                                                        output_examples=True, no_cache=True)
    ...
    def calculate_results(self, truth, predictions):
        ...
        for q_id, answer in truth_dict.items():
            if predictions[q_id].strip() == answer.strip():
                ...
                incorrect += 1
                incorrect_text[q_id] = {'truth': answer, 'predicted': predictions[q_id],
