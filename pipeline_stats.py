...
from frontend import FrontendAppConfig



def get_verdict(text):
    out, prob_val = FrontendAppConfig.text_FNF_classify(text)
    prob = np.round(prob_val, 2)

    # choice = ['fake','real']
    # out = sample(choice,1)[0]
    # if(out == 'fake'):
    #     prob = uniform(0,0.5)
    # else:
    #     prob = uniform(0.5,1)
...
for index in range(0,len(state_list)):
    ...
    print(file)
    res_list, prob_list = FrontendConfig.text_FNF_classify(df['text'].tolist())
    df['Verdict'] = res_list
    df['Prob'] = prob_list
